...
from ..tuning_modules import ConvAdapter
...
class ConvNeXtBlock(BaseModule):
    ...
    def __init__(self,
        ...
        self.tuning_module = ConvAdapter(in_channels, in_channels,
                                         kernel_size=7,
                                         padding=3,
                                         width=int(in_channels // 8),
                                         stride=1,
...
class ConvNeXtWithConvAdapter(BaseModule):
    """ConvNeXt v1&v2 backbone with Conv_Adapter.
    ...
    def __init__(self,
        ...
        for i in range(self.num_stages):
            ...
            if i >= 1:
                ...
                self.tuning_module_downsample.append(ConvAdapter(self.channels[i - 1], channels,
                                                                 kernel_size=2,
                                                                 padding=0,
                                                                 width=int(self.channels[i - 1] // 8),
                                                                 stride=2,
                                                                 groups=int(self.channels[i - 1] // 8),
                                                                 dilation=1,
                                                                 act_layer=nn.GELU))
    ...
    def init_weights(self):
        ...
        if self.init_cfg is None:
            ...
            for name, param in self.named_parameters():
                if 'tuning_module' not in name and 'norm' not in name:
                    param.requires_grad = False

    def forward(self, x):
        outs = []
        for i, stage in enumerate(self.stages):
            if isinstance(self.downsample_layers[i], nn.Conv2d) and i >= 1:
    ...
    def train(self, mode=True):
        super(ConvNeXtWithConvAdapter, self).train(mode)
        self._freeze_stages()
