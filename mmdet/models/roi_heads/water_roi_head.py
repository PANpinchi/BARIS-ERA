...
class WaterRoIHead(StandardRoIHead):
    ...
    def _mask_forward_train(self, x, sampling_results, bbox_feats, gt_masks, img_metas):
        ...
        loss_mask = self.mask_head.loss(mask_results['stage_lcf_preds'], stage_mask_targets, torch.cat(pos_labels))
    ...
    def simple_test_mask(self, x, img_metas, det_bboxes, det_labels, rescale=False):
        if self.mask_head.loss_ce is not None:
            return self.mask_rcnn_test_mask(x, img_metas, det_bboxes, det_labels, rescale=rescale)
    ...
    def mask_rcnn_test_mask(self, x, img_metas, det_bboxes, det_labels, rescale=False):
        """Simple test for mask head without augmentation."""
        ...
        if all(det_bbox.shape[0] == 0 for det_bbox in det_bboxes):
            ...
            mask_rois = bbox2roi(_bboxes)
            mask_results = self._mask_forward(x, mask_rois, torch.cat(det_labels))
            mask_pred = mask_results['stage_lcf_preds'][-1]
            # split batch mask prediction back to each image
            num_mask_roi_per_img = [len(det_bbox) for det_bbox in det_bboxes]
