...
class WaterMaskHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.loss_lcf = build_loss(loss_cfg) if loss_cfg is not None else None
    ...
    def forward(self, lcf_feats, gff_feat, rois, roi_labels):
        ...
        for idx, stage in enumerate(self.stages):
            lcf_logits = self.stage_lcf_logits[idx](lcf_feats)
            upsample_flag = self.pre_upsample_last_stage or idx < len(self.stages) - 1
            lcf_feats = stage(
                lcf_feats, lcf_logits[torch.arange(len(rois)), roi_labels][:, None], gff_feat, rois, upsample_flag)
            if self.loss_ce is not None:
                stage_lcf_preds.append(lcf_logits)
            else:
                stage_lcf_preds.append(lcf_logits[torch.arange(len(rois)), roi_labels][:, None])

        # if use class-agnostic classifier for the last stage
        if self.classes_num_in_stages[-1] == 1:
            roi_labels = roi_labels.clamp(max=0)

        lcf_preds = self.stage_lcf_logits[-1](lcf_feats)
        if self.loss_ce is not None:
            ...
            if not self.pre_upsample_last_stage:
                lcf_preds = F.interpolate(
                    lcf_preds[torch.arange(len(rois)), roi_labels][:, None], scale_factor=2, mode='bilinear', align_corners=True)
            stage_lcf_preds.append(lcf_preds)
    ...
    def loss(self, stage_lcf_preds, stage_lcf_targets, pos_labels):
        if self.loss_ce is not None:
            loss_lcf = self.loss_ce(stage_lcf_preds[-1], stage_lcf_targets[-1], pos_labels)
        else:
