...
class ActiveBoundaryLoss(nn.Module):
    def __init__(self, isdetach=True, max_N_ratio=1 / 100, ignore_label=255, label_smoothing=0.2, weight=None,
                 max_clip_dist=20., use_mask=False, loss_weight=1.0,):
    ...
    def logits2boundary(self, logit):
        ...
        while True:  # avoid the case that full image is the same color
            ...
            if kl_combine_bin.sum() > max_N:
                ...
                break
        # dilate
        dilate_weight = torch.ones((1, 1, 3, 3)).to(kl_combine_bin.device)
    ...
    def get_direction_gt_predkl(self, pred_dist_map, pred_bound, logits):
        ...
        dist_maps = torch.zeros((0, len(x))).to(pred_dist_map.device)  # 8k
        kl_maps = torch.zeros((0, len(x))).to(pred_dist_map.device)  # 8k
    ...
    def forward(self, logits, target, weight=None, **kwargs):
        eps = 1e-10
        ...
        dist_maps = self.get_dist_maps(gt_boundary).to(logits.device)  # <-- it will slow down the training, you can put it to dataloader.

        pred_boundary = self.logits2boundary(logits)
        if pred_boundary.sum() < 1:  # avoid nan
            return torch.tensor(0.0, device=logits.device, dtype=logits.dtype)  # you should check in the outside. if None, skip this loss.
